          ...
          {notifications.map((n) => (
            <li
              ...
              className={`flex animate-fade-in-up gap-3 rounded-2xl border p-4 shadow-card transition ${
                n.read ? 'border-slate-200 bg-white' : 'border-brand-100 bg-brand-50'
              }`}
            >
              <NotificationIcon type={n.type} className="mt-0.5 h-5 w-5 shrink-0 text-slate-500" />
              <div className="flex-1">
                <div className="flex items-center justify-between gap-2">
                  <Link to={notificationHref(n.type)} className="group flex-1">
                    <h2 className="text-sm font-semibold text-slate-800 group-hover:text-brand-700">
                      {n.title}
                    </h2>
                  </Link>
