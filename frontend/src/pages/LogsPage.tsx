...
import { LOG_ACTION_BADGE, LOG_ACTION_LABELS, LOG_ACTION_ORDER } from '../lib/logs';
...
export function LogsPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-xl font-bold text-slate-900">Registro de actividad</h1>
        <p className="text-sm text-slate-500">
          Auditoría de las acciones de la plataforma. Solo visible para el administrador.
        </p>
      </div>
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-card">
            {data.logs.length === 0 ? (
              <div className="flex flex-col items-center gap-2 p-10 text-center">
                ...
                <p className="text-sm text-slate-500">Sin registros para estos filtros.</p>
              </div>
            ) : (
                  ...
                  {data.logs.map((log) => (
                    <tr key={log.id} className="align-top hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {log.actorRole && (
                          <div className="text-xs text-slate-400">{ROLE_LABELS[log.actorRole]}</div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <span
