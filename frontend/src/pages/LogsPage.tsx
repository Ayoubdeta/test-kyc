...
export function LogsPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="text-xl font-bold text-slate-900 dark:text-slate-50">{t('log.pageTitle')}</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">{t('log.subtitle')}</p>
      </div>
      ...
      <div className="mb-6 animate-fade-in-up rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-card">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="font-medium text-slate-700 dark:text-slate-200">{t('log.action')}</span>
            <select
              value={draft.action ?? ''}
              onChange={(e) =>
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="font-medium text-slate-700 dark:text-slate-200">{t('log.search')}</span>
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-card">
            {data.logs.length === 0 ? (
              <div className="flex flex-col items-center gap-2 p-10 text-center">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">{t('log.empty')}</p>
              </div>
            ) : (
                    ...
                    <tr key={log.id} className="align-top hover:bg-slate-50 dark:hover:bg-slate-800">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium text-slate-800 dark:text-slate-100">
                          {log.actorLabel ?? <span className="text-slate-400 dark:text-slate-500">—</span>}
                        </div>
                        {log.actorRole && (
                          <div className="text-xs text-slate-400 dark:text-slate-500">{roleLabel(t, log.actorRole)}</div>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">
                        {log.description ?? <span className="text-slate-400 dark:text-slate-500">—</span>}
                      </td>
