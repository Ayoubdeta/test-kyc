  ...
  return (
    <DashboardLayout>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-slate-50">{t('docs.title')}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('docs.subtitle')}</p>
        </div>
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2 text-sm shadow-card">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {DOCUMENT_TYPES.map((type) => {
            ...
            return (
              <div
                key={type.key}
                className="flex animate-fade-in-up flex-col rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5 shadow-card"
              >
                <div className="flex items-start gap-3">
                  <DocTypeIcon docType={type.key} className="h-6 w-6 shrink-0 text-brand-600 dark:text-brand-400" />
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1.5">
                      {doc ? (
                        ...
                        <span className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-xs font-semibold text-slate-500 dark:text-slate-400">
                ...
                <div className="mt-3 min-h-[1.25rem] text-xs text-slate-500 dark:text-slate-400">
                  ...
                  {doc?.status === 'en_revision' && (
                    <span className="text-blue-700 dark:text-blue-400">{t('docs.inReview')}</span>
                  )}
                  {doc?.status === 'pendiente_aprobacion' && (
                    <span className="text-indigo-700 dark:text-indigo-400">{t('docs.pendingApprovalByDireccion')}</span>
                  )}
                </div>
