...
export function PdfViewerModal({
  ...
  return createPortal(
    ...
    >
      <div
        className="flex h-[90vh] w-full max-w-4xl animate-scale-in flex-col overflow-hidden rounded-2xl bg-white dark:bg-slate-900 shadow-elevated"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Cabecera */}
        <div className="flex items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-700 px-5 py-3">
          <h2 className="truncate text-sm font-semibold text-slate-800 dark:text-slate-100" title={title}>
            {title}
          </h2>
