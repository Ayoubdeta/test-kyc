...
interface KpiCardProps {
  ...
  accent?: string;
  /** Clase de fondo del chip del icono (p. ej. "bg-brand-50 dark:bg-brand-500/10"). */
  iconBg?: string;
...
export function KpiCard({
  ...
  const base =
    'group relative animate-fade-in-up overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-card';
  const content = (
    <>
  ...
  if (to) {
    return (
      <Link
        to={to}
        className={`${base} block transition duration-200 hover:-translate-y-1 hover:border-brand-200 dark:hover:border-brand-500/30 hover:shadow-elevated`}
      >
        {content}
      </Link>
    );
    ...
    <div className="flex flex-wrap items-center gap-6">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label="Distribución por estado">
        <g transform={`rotate(-90 ${center} ${center})`}>
          {total === 0 ? (
            <circle cx={center} cy={center} r={radius} fill="none" className="stroke-slate-200 dark:stroke-slate-700" strokeWidth={thickness} />
          ) : (
            segments
              .filter((s) => s.value > 0)
              .map((seg) => {
        ...
        <text x={center} y={center - 4} textAnchor="middle" className="fill-slate-900 dark:fill-slate-50" style={{ fontSize: 26, fontWeight: 700 }}>
          {total}
        </text>
        <text x={center} y={center + 16} textAnchor="middle" className="fill-slate-400 dark:fill-slate-500" style={{ fontSize: 11 }}>
          documentos
        </text>
      </svg>
      ...
      <ul className="flex flex-col gap-1.5 text-sm">
        {segments.map((seg) => (
          <li key={seg.label} className="flex items-center gap-2">
            ...
            <span className="ms-auto font-semibold text-slate-800 dark:text-slate-100">{seg.value}</span>
          </li>
        ))}
      </ul>
    </div>
    ...
    <ul className="flex flex-col gap-3">
      {items.map((it) => (
        <li key={it.label} className="grid grid-cols-[10rem_1fr_2.5rem] items-center gap-3 text-sm">
          <span className="truncate text-slate-600 dark:text-slate-300" title={it.label}>
            {it.label}
          </span>
          <span className="h-3 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
            <span
              className="block h-full rounded-full transition-all"
              style={{ width: `${(it.value / max) * 100}%`, backgroundColor: it.color ?? '#d7001b' }}
            />
          </span>
          <span className="text-end font-semibold text-slate-800 dark:text-slate-100">{it.value}</span>
        </li>
      ))}
    </ul>
  );
  ...
  return (
    <div>
      <div className="flex h-40 items-end gap-1.5">
        {items.map((it) => (
          <div key={it.label} className="flex flex-1 flex-col items-center justify-end gap-1">
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400">{it.value > 0 ? it.value : ''}</span>
            <div
      ...
      <div className="mt-1 flex gap-1.5">
        {items.map((it) => (
          <span key={it.label} className="flex-1 text-center text-[10px] text-slate-400 dark:text-slate-500">
            {it.label}
          </span>
        ))}
      </div>
      {!hasData && <p className="mt-2 text-center text-xs text-slate-400 dark:text-slate-500">Sin documentos en el periodo.</p>}
    </div>
  );
}
