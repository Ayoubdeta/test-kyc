...
interface TimelineStep {
  // Clave i18n de la etiqueta de la fase (se traduce al renderizar).
  labelKey: string;
  tone: NodeTone;
  date?: string | null;
}
...
function buildSteps(doc?: DocumentItem): TimelineStep[] {
  if (!doc) {
    return [
      { labelKey: 'timeline.uploaded', tone: 'pending' },
      { labelKey: 'timeline.inReview', tone: 'pending' },
      { labelKey: 'timeline.pendingApproval', tone: 'pending' },
      { labelKey: 'timeline.result', tone: 'pending' },
    ];
  }
  ...
  const subido: TimelineStep = { labelKey: 'timeline.uploaded', tone: 'done', date: doc.uploadedAt };
  ...
  const pendienteAprobacion: TimelineStep = {
    labelKey: 'timeline.pendingApproval',
    tone: apTone,
  ...
  let resLabelKey = 'timeline.result';
  let resDate: string | null = null;
  if (s === 'aprobado') {
    resTone = 'ok';
    resLabelKey = 'status.aprobado';
    resDate = doc.decidedAt;
  } else if (s === 'caducado') {
    resTone = 'warn';
    resLabelKey = 'status.caducado';
    resDate = doc.expiresAt;
  } else if (s === 'rechazado') {
    resTone = 'error';
    resLabelKey = 'status.rechazado';
    resDate = doc.decidedAt ?? doc.reviewedAt;
  }
  const resultado: TimelineStep = { labelKey: resLabelKey, tone: resTone, date: resDate };
...
export function DocumentTimeline({ docType, label, doc }: Props) {
  ...
  const upload = useMutation({
    ...
    onError: (err) => setUploadError(getApiErrorMessage(err, t('docs.uploadError'))),
  });
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
      ...
      <div className="flex items-start overflow-x-auto pb-1">
        {steps.map((step, i) => (
          <div key={step.labelKey} className="flex flex-1 items-start">
            {i > 0 && (
              <div
      ...
      {doc?.status === 'rechazado' && (
        <div className="mt-2 flex flex-col gap-2">
          ...
          >
            ...
            {upload.isPending ? t('timeline.uploading') : t('timeline.resend')}
          </button>
          <input
            ref={inputRef}
            type="file"
